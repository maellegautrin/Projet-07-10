...
let rec eval_add_bin a b = match (a, b) with
  |Atom s1, Atom s2 ->  Atom (string_of_int ((int_of_string s1)+(int_of_string s2)))
  |(_, _)-> raise (Error "")

let rec eval_add a b= match a with
  |Atom s1 ->( match b with
    |Atom s2 -> Atom (string_of_int ((int_of_string s1)+(int_of_string s2)))
    |Cons _-> raise (Error ""))
  |Cons(car1,cdr1)->( match b with
    |Atom _-> raise(Error "")
    |Cons(car2,cdr2)-> Cons(eval_add car1 car2, eval_add cdr1 cdr2))

let eval_sous a b= match a with
  |Atom s1 ->( match b with
    |Atom s2 -> Atom (string_of_int ((int_of_string s1)-(int_of_string s2)))
    |Cons _-> raise (Error ""))
  |Cons(car1,cdr1)->( match b with
...
let eval_fois a b= match a with
  |Atom s1 ->( match b with
    |Atom s2 -> Atom (string_of_int ((int_of_string s1)*(int_of_string s2)))
    |Cons _-> raise (Error ""))
  |Cons(car1,cdr1)->( match b with
...
let rec eval_add_list arg=match arg with
  |Atom _ -> arg
  |Cons(car,cdr)-> let Atom evcar=eval_add_list car in 
                   let Atom evcdr=eval_add_list cdr in
    Atom string_of_int ((int_of_string evcar) + (int_of_string evcdr))


let eval_plugrand arg1 arg2 =
  let rec plugrand a b =
    match a with
      Atom s1 ->
      begin match b with
	  Atom s2 -> s1 >= s2
    ...
    | Cons (car1,cdr1) ->
      begin match b with
	  Atom _ -> false
	| Cons (car2,cdr2) -> plugrand car1 car2 && plugrand cdr1 cdr2
      end
  in

  if plugrand arg1 arg2
  then Atom "t"
  else Atom "nil"

let eval_plupetit arg1 arg2 =
  let rec plupetit a b =
    match a with
      Atom s1 ->
      begin match b with
	  Atom s2 -> s1 <= s2
    ...
    | Cons (car1,cdr1) ->
      begin match b with
	  Atom _ -> false
	| Cons (car2,cdr2) -> plupetit car1 car2 && plupetit cdr1 cdr2
      end
  in

  if plupetit arg1 arg2
  then Atom "t"
  else Atom "nil"
...
let all_builtins =
  [ (* Special forms *)
    ...
    "sous", BuiltinFn2 eval_sous;
    "fois", BuiltinFn2 eval_fois;
    "plugrand", BuiltinFn2 eval_plugrand;
    "plupetit", BuiltinFn2 eval_plupetit;
